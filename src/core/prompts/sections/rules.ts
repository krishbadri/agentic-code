...
- **CRITICAL for implementing new features or classes:** Before writing ANY new code that implements an interface, protocol, or abstract class:
  1. ALWAYS use read_file to examine the existing interface/protocol/type definitions first
  2. Study existing implementations in the codebase to understand the expected patterns
  3. Match the exact method signatures, parameter types, and return types of existing code
  4. If the task mentions implementing something like "ResultStore" or any named interface, find and read that interface definition BEFORE implementing
  5. Run tests after implementation to verify correctness - if tests fail, read the error messages carefully and fix the root cause
